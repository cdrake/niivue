...
void main(void) {
 ...
 samp += texture(intensityVol,vx+vec3(-dX,-dY,+dZ));
 samp += texture(intensityVol,vx+vec3(-dX,-dY,-dZ));
 FragColor = uvec4(samp*0.125*65535.0);
}`
...
//  FragColor = uvec4(0.0357*65536.0*blurred);
// }`
...
void main(void) {
  vec3 vx = vec3(TexCoord.xy, coordZ);
  //Neighboring voxels 'T'op/'B'ottom, 'A'nterior/'P'osterior, 'R'ight/'L'eft
  float TAR = float(texture(intensityVol,vx+vec3(+dX,+dY,+dZ)).r);
  float TAL = float(texture(intensityVol,vx+vec3(+dX,+dY,-dZ)).r);
  float TPR = float(texture(intensityVol,vx+vec3(+dX,-dY,+dZ)).r);
  float TPL = float(texture(intensityVol,vx+vec3(+dX,-dY,-dZ)).r);
  float BAR = float(texture(intensityVol,vx+vec3(-dX,+dY,+dZ)).r);
  float BAL = float(texture(intensityVol,vx+vec3(-dX,+dY,-dZ)).r);
  float BPR = float(texture(intensityVol,vx+vec3(-dX,-dY,+dZ)).r);
  float BPL = float(texture(intensityVol,vx+vec3(-dX,-dY,-dZ)).r);
  vec4 gradientSample = vec4 (0.0, 0.0, 0.0, 0.0);
  gradientSample.r = BAR+BAL+BPR+BPL -TAR-TAL-TPR-TPL;
  gradientSample.g = TPR+TPL+BPR+BPL -TAR-TAL-BAR-BAL;
  gradientSample.b = TAL+TPL+BAL+BPL -TAR-TPR-BAR-BPR;
